...
/* ZSTD_seekable_customFile implementation that reads/seeks a buffer while
 * keeping track of total bytes read */
typedef struct {
  const void *ptr;
  size_t size;
  size_t pos;
...
int main(int argc, const char **argv) {
  ...
  printf("Test %u - simple round trip: ", testNb++);
  {
    ...
    {
      size_t const initStatus = ZSTD_seekable_initCStream(
    ...
    assert(stream != NULL);
    {
      size_t const initStatus =
          ZSTD_seekable_initBuff(stream, seekBuffer, seekSize);
      assert(!ZSTD_isError(initStatus));
    }

    {
      size_t const decStatus =
          ZSTD_seekable_decompress(stream, outBuffer, outCapacity, 0);
    ...
    unsigned long long const frame0Offset =
        ZSTD_seekTable_getFrameCompressedOffset(zst, 0);
    assert(frame0Offset == 0);

    unsigned long long const content0Offset =
        ZSTD_seekTable_getFrameDecompressedOffset(zst, 0);
  ...
  { /* Github issue #2335 */
    ...
    {
      size_t const status =
          ZSTD_seekable_initBuff(stream, compressed_data, compressed_size);
    ...
    /* Should return an error, but not hang */
    {
      ...
      size_t const status = ZSTD_seekable_decompress(stream, uncompressed_data,
  ...
  { /* Github issue #FIXME */
    ...
    const uint8_t compressed_data[27] = {
        ...
        (uint8_t)'\x00', (uint8_t)'\x32', (uint8_t)'\x91', (uint8_t)'\x00',
        (uint8_t)'\x00', (uint8_t)'\x00', (uint8_t)'\x5e', (uint8_t)'\x2a',
        (uint8_t)'\x4d', (uint8_t)'\x18', (uint8_t)'\x09', (uint8_t)'\x00',
        (uint8_t)'\x00', (uint8_t)'\x00', (uint8_t)'\x00', (uint8_t)'\x00',
        (uint8_t)'\x00', (uint8_t)'\x00', (uint8_t)'\x00', (uint8_t)'\xb1',
    ...
    const size_t offset = 2;
    /* Should return an error, but not hang */
    status = ZSTD_seekable_decompress(stream, uncompressed_data,
                                      uncompressed_size, offset);
    if (!ZSTD_isError(status)) {
      ZSTD_seekable_free(stream);
      goto _test_error;
  ...
  { // compressing empty string should return a zstd header
    ...
    if ((((char *)output.dst)[0] != '\x28') |
        (((char *)output.dst)[1] != '\xb5') |
        (((char *)output.dst)[2] != '\x2f') |
        (((char *)output.dst)[3] != '\xfd')) {
      printf("%#02x %#02x %#02x %#02x\n", ((char *)output.dst)[0],
             ((char *)output.dst)[1], ((char *)output.dst)[2],
             ((char *)output.dst)[3]);
  ...
  {
    char const inBuffer[] = "Lorem ipsum dolor sit amet, consectetur "
    ...
    /* compress test data with a small frame size to ensure multiple frames in
     * the output */
    unsigned const maxFrameSize = 40;
    {
      size_t const initStatus = ZSTD_seekable_initCStream(
          zscs, 9, 0 /* checksumFlag */, maxFrameSize);
      assert(!ZSTD_isError(initStatus));
    }
    ...
    {
      ZSTD_seekable_customFile srcFile = {&buffWrapper, &readBuffWithTotal,
                                          &seekBuffWithTotal};
      size_t const initStatus = ZSTD_seekable_initAdvanced(stream, srcFile);
      assert(!ZSTD_isError(initStatus));
    }

    /* Perform a series of small reads and seeks (repeatedly read 1 byte and
       skip 1 byte) and check that we didn't reread input data unnecessarily */
    size_t pos;
    for (pos = 0; pos < inSize; pos += 2) {
      size_t const decStatus =
          ZSTD_seekable_decompress(stream, outBuffer, 1, pos);
    ...
    for (idx = 0; idx < sizeof(tests) / sizeof(tests[0]); idx++) {
      size_t const decStatus = ZSTD_seekable_decompress(
          stream, outBuffer, tests[idx].size, tests[idx].offset);
      if (decStatus != tests[idx].size ||
          memcmp(outBuffer, inBuffer + tests[idx].offset, tests[idx].size) !=
              0) {
        free(seekBuffer);
        free(outBuffer);
        ZSTD_seekable_freeCStream(zscs);
  ...
  { /* Compressing and decompressing zero bytes should work correctly */
    ...
    {
      size_t const initStatus = ZSTD_seekable_initCStream(zscs, 1, 0, 1024);
      assert(!ZSTD_isError(initStatus));
    }

    {
      ZSTD_outBuffer outb = {.dst = seekBuffer, .pos = 0, .size = seekCapacity};
      ZSTD_inBuffer inb = {.src = NULL, .pos = 0, .size = 0};
    ...
    /* Decompressing 0 bytes at offset 0 should return 0 */
    {
      size_t const decStatus =
          ZSTD_seekable_decompress(stream, outBuffer, 0, 0);
      assert(decStatus == 0);
    }
  ...
  printf("Test %u - round trip with checksum enabled: ", testNb++);
  { /* Compress and decompress with checksumFlag=1 to exercise that path */
    size_t const inSize = 2000;
    void *const inBuffer = malloc(inSize);
    assert(inBuffer != NULL);
    memset(inBuffer, 'A', inSize);

    size_t const seekCapacity = 4000;
    void *const seekBuffer = malloc(seekCapacity);
    assert(seekBuffer != NULL);
    size_t seekSize;
    ...
    {
      size_t const initStatus = ZSTD_seekable_initCStream(
          zscs, 5, 1 /* checksumFlag */, (unsigned)inSize);
      assert(!ZSTD_isError(initStatus));
    }
    ...
    assert(memcmp(inBuffer, outBuffer, inSize) == 0);
  ...
  printf("Test %u - out-of-bounds decompress offset returns error: ", testNb++);
  { /* Decompressing at an offset beyond data should return an error */
    size_t const inSize = 500;
    void *const inBuffer = malloc(inSize);
    assert(inBuffer != NULL);
    memset(inBuffer, 'B', inSize);

    size_t const seekCapacity = 2000;
    void *const seekBuffer = malloc(seekCapacity);
    assert(seekBuffer != NULL);
    size_t seekSize;
    ...
    {
      size_t const initStatus =
          ZSTD_seekable_initCStream(zscs, 1, 0, (unsigned)inSize);
      assert(!ZSTD_isError(initStatus));
    }
    ...
    /* Attempt to decompress 1 byte at offset == inSize (past the end) */
    {
      char outByte;
      size_t const decStatus =
          ZSTD_seekable_decompress(stream, &outByte, 1, inSize);
      /* Should either return 0 (no data) or an error */
      assert(decStatus == 0 || ZSTD_isError(decStatus));
    }
  ...
  printf("Test %u - invalid frame index returns error: ", testNb++);
  { /* Accessing a frame index beyond the number of frames should error */
    size_t const inSize = 1000;
    void *const inBuffer = malloc(inSize);
    assert(inBuffer != NULL);
    memset(inBuffer, 'C', inSize);

    size_t const seekCapacity = 3000;
    void *const seekBuffer = malloc(seekCapacity);
    assert(seekBuffer != NULL);
    size_t seekSize;
    ...
    {
      unsigned const nbFrames = ZSTD_seekable_getNumFrames(stream);
      /* Accessing frame index well past valid range should error.
       * Note: the API uses `frameIndex > tableLen` for bounds checking,
       * so we use nbFrames + 1 to reliably trigger the error path. */
      unsigned const badIdx = nbFrames + 1;
      size_t const cSize = ZSTD_seekable_getFrameCompressedSize(stream, badIdx);
      assert(ZSTD_isError(cSize));
      size_t const dSize =
          ZSTD_seekable_getFrameDecompressedSize(stream, badIdx);
      assert(ZSTD_isError(dSize));
      unsigned long long const cOff =
          ZSTD_seekable_getFrameCompressedOffset(stream, badIdx);
      assert(cOff == ZSTD_SEEKABLE_FRAMEINDEX_TOOLARGE);
    }
