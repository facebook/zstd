...
static size_t ZSTD_resetCCtx_internal(ZSTD_CCtx *zc,
...

  {
    ...
    /* ldm hash table */
    if (params->ldmParams.enableLdm == ZSTD_ps_enable) {
      /* Note: memset is used here to zero the LDM hash table.
       * An alternative would be a generation counter to lazily
       * invalidate entries, avoiding the upfront memset cost.
       * However, LDM is only enabled with --long mode (rare in
       * default paths), and the table allocation itself dominates
       * this cost. The branch overhead of a generation check on
       * every LDM lookup would likely negate the memset savings.
       * Keeping the memset is the simpler and safer choice.
       */
    ...
    if (params->ldmParams.enableLdm == ZSTD_ps_enable) {
      /* Note: same rationale as LDM hash table above — memset
       * is acceptable here because LDM is rarely enabled, and
       * bucket offset tables are small relative to hash tables.
       */
