                  ...
                  int const optLevel)
{
    ...
    if (optPtr->litLengthSum == 0) {  /* no literals stats collected -> first block assumed -> init */
        ...
        /* The dictionary's FSE tables are only complete when every repeat
         * mode is valid.  A table with missing symbols returns a fake cost
         * of tableLog + 1 from FSE_getMaxNbBits(), which is not a valid input
         * to the scaled frequency calculation below. */
        if (optPtr->symbolCosts->huf.repeatMode == HUF_repeat_valid &&
            optPtr->symbolCosts->fse.litlength_repeatMode == FSE_repeat_valid &&
            optPtr->symbolCosts->fse.matchlength_repeatMode == FSE_repeat_valid &&
            optPtr->symbolCosts->fse.offcode_repeatMode == FSE_repeat_valid) {
