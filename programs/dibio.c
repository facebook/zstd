...
static int DiB_loadFiles(void *buffer, size_t *bufferSizePtr,
  ...
  while (nbSamplesLoaded < sstSize && fileIndex < nbFiles) {
    ...
    if (f == NULL)
      EXM_THROW(10, "zstd: dictBuilder: %s %s ", fileNamesTable[fileIndex],
                strerror(errno));
 ...
 * set is very large, it will load random elements from it, instead of just the
 * first ones. */
 ...
 * sizes table.
 */
static fileStats DiB_fileStats(const char **fileNamesTable, int nbFiles,
                               size_t chunkSize, int displayLevel) {
  fileStats fs;
  int n;
  ...
  for (n = 0; n < nbFiles; n++) {
    ...
    /* Skip files that are too small to provide meaningful training data.
     * Files smaller than 8 bytes add negligible value to dictionary
     * training and may cause edge-case issues. */
    if (fileSize > 0 && fileSize < 8) {
      DISPLAYLEVEL(3, "Sample file '%s' is too small (%d bytes), skipping...\n",
                   fileNamesTable[n], (int)fileSize);
      continue;
    }
    ...
    if (fileSize <= 0) {
      if (fileSize < 0) {
        DISPLAYLEVEL(
            3, "Sample file '%s' is unreadable or stat failed, skipping...\n",
            fileNamesTable[n]);
      } else {
    ...
    if (chunkSize > 0) {
      /* Samples smaller than 8 bytes provide negligible training value */
      if ((U64)fileSize < 8) {
        DISPLAYLEVEL(3,
                     "Sample file '%s' is too small (%d bytes), skipping...\n",
                     fileNamesTable[n], (int)fileSize);
        continue;
      }
  ...
  DISPLAYLEVEL(4, "Found training data %d files, %d KB, %d samples\n", nbFiles,
               (int)(fs.totalSizeToLoad / (1 KB)), fs.nbSamples);
  return fs;
}
...
int DiB_trainFromFiles(const char *dictFileName, size_t maxDictSize,
                       ...
                       unsigned memLimit) {
  fileStats fs;
  size_t *sampleSizes; /* vector of sample sizes. Each sample can be up to
                          SAMPLESIZE_MAX */
...

  {
    ...
    /* TODO: there is opportunity to stop DiB_fileStats() early when the data
     * limit is reached */
    loadedSize =
        (size_t)MIN(MIN((S64)maxMem, fs.totalSizeToLoad), MAX_SAMPLES_SIZE);
    if (memLimit != 0) {
      DISPLAYLEVEL(2,
                   "!  Warning : setting manual memory limit for dictionary "
                   "training data at %u MB \n",
  ...
  if (fs.nbSamples < 5) {
    ...
    DISPLAYLEVEL(2, "!  Alternatively, split file(s) into fixed-size samples, "
                    "with --split=#\n");
    EXM_THROW(14,
              "nb of samples too low"); /* we now clearly forbid this case */
  }
  if (fs.totalSizeToLoad < (S64)maxDictSize * 8) {
    DISPLAYLEVEL(2, "!  Warning : data size of samples too small for target "
                    "dictionary size \n");
    DISPLAYLEVEL(2, "!  Samples should be about 100x larger than target "
                    "dictionary size \n");
  }
  ...
  if ((S64)loadedSize < fs.totalSizeToLoad)
    DISPLAYLEVEL(
        1,
        "Training samples set too large (%u MB); training on %u MB only...\n",
...

  {
    ...
    if (params) {
      ...
      dictSize =
          ZDICT_trainFromBuffer_legacy(dictBuffer, maxDictSize, srcBuffer,
                                       sampleSizes, nbSamplesLoaded, *params);
    } else if (coverParams) {
      if (optimize) {
        dictSize = ZDICT_optimizeTrainFromBuffer_cover(
            dictBuffer, maxDictSize, srcBuffer, sampleSizes, nbSamplesLoaded,
            coverParams);
