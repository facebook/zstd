...
static void usage_advanced(const char* programName)
{
    ...
    DISPLAYOUT( "--row-params : use new set of cparams that include row-based matchfinder \n");
...
int main(int const argCount, const char* argv[])
{
    ...
    for (argNb=1; argNb<argCount; argNb++) {
        ...
        if (argument[0]=='-') {
            ...
            if (argument[1]=='-') {
                ...
                if (!strcmp(argument, "--row-params")) { useRowParams = 1; continue; }
    ...
    if (operation==zom_bench) {
        ...
        benchParams.useRowParams = useRowParams;
    ...
    if (operation==zom_compress) {
        ...
        FIO_setUseRowParams(prefs, (unsigned)useRowParams);
...
#else
        (void)contentSize; (void)suffix; (void)adapt; (void)rsyncable; (void)ultra; (void)cLevel; (void)ldmFlag; (void)literalCompressionMode; (void)targetCBlockSize; (void)streamSrcSize; (void)srcSizeHint; (void)ZSTD_strategyMap; (void)useRowParams; /* not used when ZSTD_NOCOMPRESS set */
